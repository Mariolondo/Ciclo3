    ...
    try:
        ...
        if request.method == 'POST':    
            ...
            if db.execute('SELECT id FROM user WHERE username = ?', (username,)).fetchone() is not None:
                error = 'User {} ya fue registrado.'.format(username)
                flash(error)
                return render_template('auth/register.html')
            ...
            if(not utils.isPasswordValid(password)):
                error = 'La contraseña debe contener al menos una letra minúscula, una letra mayúscula y un número, esta debe contener al menos 8 caracteres'
                flash(error)
                return render_template('auth/register.html')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                content = 'Hola Para cambiar tu contraseña sigue en el siguiente link ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
